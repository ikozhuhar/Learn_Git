...
1. --pretty="..." — определяет формат вывода.
2. %h — укороченный хеш коммита.
3. %ad — дата коммита.
4. | — просто визуальный разделитель.
5. %s — комментарий.
6. %d — дополнения коммита («головы» веток или теги).
7. %an — имя автора.
8. --date=short — сохраняет формат даты коротким и симпатичным.
